      ...
      <div class="flex -space-x-2">
        <template v-for="(avatar, idx) in board.avatars" :key="idx">
          <img
            v-if="avatar.img"
            :src="avatar.img"
            class="w-7 h-7 rounded-full border-2 border-border shadow"
          />
          <span
            v-else
            class="w-7 h-7 rounded-full bg-muted text-muted-foreground flex items-center justify-center text-xs font-bold border-2 border-border shadow"
          >
            {{ avatar.initials }}
          </span>
        </template>
